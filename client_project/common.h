...
void __rm_path(char *);
char config_file[100] = "/home/moying12138/client_project/config";
...
int socket_creat(int port) {
    ...
    setsockopt(sockfd, SOL_SOCKET, SO_LINGER, (const char *)&m_lin, sizeof(struct linger));
...
void __mv_path(char *oldpath, char *newpath)  {
    char operatorx[200] = "cp ";
    strcat(operatorx, oldpath);
    strcat(operatorx, " ");
    strcat(operatorx, newpath);
    //strcat(operatorx, " 2> /dev/null");
    system(operatorx);
}

void __backup_system(char *filepath, char *tarpath) {
    char str[200] = "tar zcvf ";
    strcat(tarpath, ".tar ");
    strcat(str, tarpath);
    strcat(str, filepath);
    printf("str : %s\n", str);
